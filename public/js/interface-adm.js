$('document').ready(function(){
	...
	if( $.inputmask ){
		...
		$('.mask-zip-br').inputmask('99999-999');
		$('.mask-phone').inputmask('9999[9]-9999');
		$('.mask-phonearea').inputmask('(99) 9999[9]-9999', {
			"onincomplete":function(){
				var position = 6;
				var rawValue = $(this).inputmask("unmaskedvalue");
				if(rawValue.length==10){
					var newPat = [rawValue.slice(0, position), '-', rawValue.slice(position)].join('');
					$(this).val(newPat);
				}
			}
		});
	...
	var $cityPicker = $('.city-picker.typeahead');
	if( $statePicker.length>0 || $cityPicker.length>0 ){
		...
		if($statePicker.attr('data-fetch')==1){
			var selected = $statePicker.attr('data-selected')==1 ? 'selected' : null;
			if($statePicker.length>0){
				ajaxRequest.endpoint(getSegment(3)+'/ajax-fetch-states').request('.state-loader').then(function(r){
					states = r.list;
					$statePicker.children().remove();
					
					for( var i in states){
						$statePicker.append('<option '+selected+'>'+states[i]+'</option>');
					}
				});
			}
		}
		...
		if( $cityPicker.length>0){
			...
			$statePicker.on('change', function(){
				ajaxRequest.endpoint(getSegment(3)+'/ajax-fetch-cities').param('state', $(this).val()).request('.cities-loader').then(function(r){
					cities = r.list;
					//$cityPicker.val('');
					$cityPicker.typeahead('destroy');
					typeahead = $cityPicker.typeahead(options, {
						name: 'cities',
